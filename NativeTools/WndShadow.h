...
public:
	...
	bool SetPosition(POINT NewOffset);
	bool SetColor(COLORREF NewColor = 0);

	int GetSize();
	unsigned int GetSharpness();
	unsigned int GetDarkness();
	POINT GetPosition();
	COLORREF GetColor();

	void SetOnlyRectangularRegions(bool set = true);
	HWND GetHWND();

protected:
	static LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
	static LRESULT CALLBACK ParentProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
	LRESULT ParentProcedure(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam, bool& isHandled);
	virtual LRESULT CallOriginalParentProcedure(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
	virtual void ConnectOriginalParentProcedure(HWND hwndParent);
	...
	bool IsDisabledByAero(HWND hParentWnd);
};
